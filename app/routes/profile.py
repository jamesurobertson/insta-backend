...
def index(id):
    ...
    followers = Follow.query.filter(Follow.user_followed_id == id).count()
    follows = Follow.query.filter(Follow.user_id == id).count()
    ...
    return {"num_posts": post_count, "posts": plist, "num_followers": followers, "num_follows": follows, "user": user.to_dict() }
